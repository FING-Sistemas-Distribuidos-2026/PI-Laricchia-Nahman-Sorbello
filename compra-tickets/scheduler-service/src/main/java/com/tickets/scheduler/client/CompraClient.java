...
public class CompraClient {
    ...
    public CompraClient(
            @Value("${compra-service.url:http://localhost:8083}")
            String compraUrl
    ) {
     ...
     *
     * El scheduler no crea active:{userId}.
     * El scheduler no toca buying:sessions.
     */
    public ActivateOutcome activateUser(UUID userId) {
        try {
            ...
            if (response == null || response.ticketId() == null) {
                log.error(
                        "[CompraClient] activate/{} devolvió response vacío",
                        userId
                );

                return new ActivateOutcome(
            ...
            log.info(
                    "[CompraClient] activate/{} OK. TicketId: {}",
                    userId,
                    response.ticketId()
            );
            ...
            log.error(
                    "[CompraClient] Error de red activate/{}: {}",
                    userId,
                    e.getMessage()
            );
    ...
    /**
     * Cantidad de usuarios actualmente en BUYING.
     *
     * El scheduler no lee Redis HASH directamente.
     * Se lo pregunta a compra-service.
     */
    public long getActiveBuyingCount() {
        try {
            Map<?, ?> response = restClient.get()
                    .uri("/api/purchase/active-count")
                    .retrieve()
                    .body(Map.class);
            ...
            log.error(
                    "[CompraClient] Error consultando active-count: {}",
                    e.getMessage()
            );
    ...
    private ActivateOutcome manejarErrorCliente(
            ClientErrorException e,
            UUID userId
    ) {
        if (e.statusCode == 404) {
            log.warn(
                    "[CompraClient] activate/{} → 404. Usuario inexistente.",
                    userId
            );

            return new ActivateOutcome(
                    ActivateResult.USER_NOT_FOUND,
                    null
            );
        }
        ...
        if (e.statusCode == 409) {
            log.warn(
                    "[CompraClient] activate/{} → 409. Sin tickets o estado incorrecto.",
                    userId
            );

            return new ActivateOutcome(
                    ActivateResult.NO_TICKETS,
                    null
            );
        }
